...
class SignUpPage:
    def __init__(self):
        ...
        self.register_greeting = ft.Text(
            "Регистрация",
            size=24,
            weight=ft.FontWeight.BOLD,
            text_align=ft.TextAlign.CENTER,
        ...
        self.reg_login_input = ft.TextField(
            label="Логин",
            width=300,
        ...
        self.reg_password_input = ft.TextField(
            label="Пароль", width=300, password=True, can_reveal_password=True, border_radius=15,
        )
        self.reg_repeat_password_input = ft.TextField(
            label="Повторите пароль", width=300, password=True, can_reveal_password=True, border_radius=15,
        )

        self.register_button = ft.ElevatedButton(
            "Зарегистрироваться",
            width=300,
            on_click=lambda _: self.page.run_task(self.registerPress, _),
        )
    ...
    async def registerPress(self, e: ft.ControlEvent):
        if self.reg_login_input.value == "":
            self.reg_login_input.focus()
            return self.errorMessage('Write login')

        if self.reg_password_input.value == "":
            self.reg_password_input.focus()
            return self.errorMessage('Write password')

        if self.reg_repeat_password_input.value == "":
            self.reg_repeat_password_input.focus()
            return self.errorMessage('Write repeat password')

        if self.reg_repeat_password_input.value != self.reg_password_input.value:
            return self.errorMessage('Wrong repeat password')

        host_api = HostApi()
        response = await host_api.signUp(username=self.reg_login_input.value,
                                         password=self.reg_password_input.value,
                                         confirm_password=self.reg_repeat_password_input.value)
        if response.content:
    ...
    def view(self, page: ft.Page, params: Params, basket: Basket):
        ...
        self.page.title = f"XYZ SignUp Page"
        ...
        inputContainer = ft.Container(
            ...
            content=ft.Column(
                [
                    self.greeting,
                    animatedBox,
                    self.register_greeting,
                    self.reg_login_input,
                    self.reg_password_input,
                    self.reg_repeat_password_input,
        ...
        central_overlay = ft.Stack(
            controls=[*circles(page=self.page), row_container],
            expand=True
        )
